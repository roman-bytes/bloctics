...
export async function loader({ request }: LoaderArgs) {
  ...
  const currentState = await bloctics.resolveState(State.create(stateConfig));
...
export const action = async ({ request, params: { state } }) => {
  ...
  const nextState = await asyncInterpret(bloctics, 3_000, currentState, event);

  console.log("NMEXT STATE", nextState);
